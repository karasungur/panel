...
router.post('/', tokenDogrula, (req, res) => {
    ...
    const sonuc = db.prepare('INSERT INTO notlar (kullanici_id, baslik, icerik) VALUES (?, ?, ?)').run(req.kullanici.id, baslik || 'Başlıksız', icerik || '');
...
router.put('/:id', tokenDogrula, (req, res) => {
    ...
    db.prepare("UPDATE notlar SET baslik = COALESCE(?, baslik), icerik = COALESCE(?, icerik), guncellenme_tarihi = CURRENT_TIMESTAMP WHERE id = ?")
        .run(baslik ?? null, icerik ?? null, req.params.id);
